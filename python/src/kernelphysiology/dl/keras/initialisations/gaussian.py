    ...
    for i, layer in enumerate(model.layers):
        if type(layer) in which_layers:
            ...
            if rows > 1:
                ...
                for d in range(dpts):
                    for c in range(chns):
                        sigmax_dc = random.uniform(0, sigmax)
                        if sigmay is not None:
                            sigmay_dc = random.uniform(0, sigmay)
                        else:
                            sigmay_dc = None
                        meanx_dc = random.uniform(-meanx, meanx)
                        meany_dc = random.uniform(-meany, meany)
                        theta_dc = random.uniform(-theta, theta)
...
def random_g1(sigma, theta, seta, width):
    sigma_dc = random.uniform(0, sigma)
    theta_dc = random.uniform(-theta, theta)
    seta_dc = random.uniform(0, seta)
