...
def get_train_dataset(pickle_file, target_size, mean_std, scale=(0.8, 1.0),
    ...
    common_transforms = [
        recursive_transforms.RandomHorizontalFlip(),
        recursive_transforms.RandomResizedCrop(target_size, scale=scale)
    ]
...
def get_validation_dataset(pickle_file, target_size, mean_std):
    ...
    target_transform = transforms.Compose([transforms.ToTensor()])
    common_transforms = [
        recursive_transforms.Resize(target_size)]
...
def _npy_loader(input_path):
    img = np.float32(np.load(input_path)).squeeze()
    ...
    for i in range(num_frames):
        ...
        for j in range(num_sensors):
            ...
            if current_img.mode == 'RGB':
                ...
                current_img = np.uint8(np.asarray(current_img).copy())
            else:
                current_img = np.float32(np.asarray(current_img).copy())
                # FIXME: this is not a proper solution
                max_depth = 10
                current_img /= max_depth
                current_img *= 255
                current_img = np.uint8(current_img)
