...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, contrast_head='l4', num_classes=1000,
        ...
        if contrast_head == 'l1':
            contrast_fcm = 1
        elif contrast_head == 'l2':
            contrast_fcm = 2
        elif contrast_head == 'l3':
            contrast_fcm = 4
        elif contrast_head == 'l4':
            contrast_fcm = 8
        else:
            raise ValueError('Unsupported contrast head %s' % contrast_head)
        self.contrast_head = contrast_head
        self.contrast_fc = nn.Linear(inplanes * contrast_fcm * block.expansion,
                                     2)
    ...
    def _contrast_output(self, x):
        x_contrast = self.avgpool(x)
        x_contrast = x_contrast.view(x_contrast.size(0), -1)
        x_contrast = self.contrast_fc(x_contrast)
        return x_contrast
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        if self.contrast_head == 'l1':
            x_contrast = self._contrast_output(x)

        x = self.layer2(x)
        if self.contrast_head == 'l2':
            x_contrast = self._contrast_output(x)

        x = self.layer3(x)
        if self.contrast_head == 'l3':
            x_contrast = self._contrast_output(x)

        x = self.layer4(x)
        if self.contrast_head == 'l4':
            x_contrast = self._contrast_output(x)
