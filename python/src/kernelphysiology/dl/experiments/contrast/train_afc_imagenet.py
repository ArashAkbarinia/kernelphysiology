...
from kernelphysiology.dl.experiments.contrast import resnet as custom_models
...
def which_architecture(network_name, customs, contrast_head):
    if customs is None:
        model = custom_models.__dict__[network_name](
            pretrained=False, pooling_type='max', in_chns=3,
            num_classes=1000, contrast_head=contrast_head,
        )
    else:
        ...
        if 'blocks' in customs and customs['blocks'] is not None:
            ...
            model = custom_models.__dict__[network_name](
                ...
                kernel_size=kernel_size, contrast_head=contrast_head,
            )
        else:
            model = custom_models.__dict__[network_name](
                pretrained=False, pooling_type=pooling_type, in_chns=in_chns,
                num_classes=num_classes, contrast_head=contrast_head,
            )
    return model
...
def main_worker(ngpus_per_node, args):
    ...
    if args.transfer_weights is not None:
        ...
        if os.path.isfile(args.network_name):
            ...
            model = which_architecture(
                checkpoint['arch'], customs, args.contrast_head
            )
            ...
            model = custom_models.__dict__[args.network_name](
                args.blocks, contrast_head=args.contrast_head,
                pooling_type=args.pooling_type, in_chns=len(mean),
                num_classes=args.num_classes, inplanes=args.num_kernels,
    ...
    args.out_dir = prepare_training.prepare_output_directories(
        dataset_name='contrast', network_name=args.network_name,
...
def extra_args_fun(parser):
    ...
    specific_group.add_argument('--contrast_head', default='l4', type=str)
