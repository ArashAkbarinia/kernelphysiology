...
__all__ = [
    'ResNet', 'resnet_basic_custom', 'resnet_bottleneck_custom'
]
...
class PredictionBlock(nn.Module):
    ...
    def __init__(self, outputs, out_type, last_fcm, inplanes, block):
        ...
        if outputs[out_type] is not None:
            ...
            if last_fcm < fcm:
                raise ValueError('This network has no %s area.' % area)

            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
            self.fc = nn.Linear(
                inplanes * fcm * block.expansion, output['num_classes']
            )
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, outputs=None,
                 zero_init_residual=False,
                 groups=1, width_per_group=64,
                 replace_stride_with_dilation=None,
                 norm_layer=None, pooling_type='max', in_chns=3, inplanes=64):
        ...
        self.fc_object = self._prediction_layer(
            'objects', fcm, inplanes, block
        )
        self.fc_munsells = self._prediction_layer(
            'munsells', fcm, inplanes, block
        )
        self.fc_illuminants = self._prediction_layer(
            'illuminants', fcm, inplanes, block
        )
    ...
    def _prediction_layer(self, out_type, last_fcm, inplanes, block):
        layers = []
        layers.append(
            PredictionBlock(
                self.outputs, out_type, last_fcm, inplanes, block
            )
        )
        return nn.Sequential(*layers)
    ...
    def forward(self, x):
        ...
        bs = [b1, b2, b3, b4]
        objects = self.fc_object(bs[self.outputs['objects']['ind']])
        munsells = self.fc_munsells(bs[self.outputs['munsells']['ind']])
        illuminants = self.fc_illuminants(
            bs[self.outputs['illuminants']['ind']]
        )
...
def resnet_basic_custom(planes, **kwargs):
    """Constructs a ResNet-Basic-Custom model."""
    return _resnet(BasicBlock, planes, **kwargs)


def resnet_bottleneck_custom(planes, **kwargs):
    """Constructs a ResNet-Basic-Custom model."""
    return _resnet(Bottleneck, planes, **kwargs)
