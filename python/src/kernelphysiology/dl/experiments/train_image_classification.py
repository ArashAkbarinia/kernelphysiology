...
def main_worker(ngpus_per_node, args):
    ...
    if args.transfer_weights is not None:
        ...
        model = resnet.__dict__[args.network_name](
            ...
            num_classes=args.num_classes, neg_params=args.neg_params
        )
    ...
    criterion_pos = nn.CrossEntropyLoss().cuda(args.gpus)
    criterion_neg = nn.BCEWithLogitsLoss().cuda(args.gpus)
    ...
    if args.num_augmentations != 0:
        augmentations = preprocessing.RandomAugmentationTransformation(
            ...
            dataset_utils.is_dataset_pil_image(args.dataset)
        )
    ...
    train_dataset = dataset_utils.get_train_dataset(
    ...
    validation_dataset = dataset_utils.get_validation_dataset(
    ...
    for epoch in range(args.initial_epoch, args.epochs):
        ...
        train_log = train_on_data(
            train_loader, model, criterion_pos, criterion_neg, optimizer,
            epoch, args
        )

        # evaluate on validation set
        validation_log = validate_on_data(
            val_loader, model, criterion_pos, args
        )
...
def train_on_data(train_loader, model, criterion_pos, criterion_neg,
    ...
    for i, (input_image, target_pos, target_neg) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)

        if args.gpus is not None:
            input_image = input_image.cuda(args.gpus, non_blocking=True)
        target_pos = target_pos.cuda(args.gpus, non_blocking=True)
        target_neg = target_neg.cuda(args.gpus, non_blocking=True)

        # compute output
        out_pos, out_negs = model(input_image)
        loss_pos = criterion_pos(out_pos, target_pos)
        loss_neg = 0
        for o_neg in out_negs:
            loss_neg += criterion_neg(o_neg, target_neg)
        if len(out_negs) > 0:
        ...
        acc1, acc5 = accuracy(out_pos, target_pos, topk=topks)
