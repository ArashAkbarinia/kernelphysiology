function [net, stats] = CnnTrainSimple(net, imdb, getBatch, varargin)
...
if ~isempty(opts.solver)
  assert(isa(opts.solver, 'function_handle') && nargout(opts.solver) == 2, 'Invalid solver; expected a function handle with two outputs.');
...
for t = 1:params.batchSize:numel(subset)
  fprintf('%s: epoch %02d: %3d/%3d:', mode, params.epoch, fix((t - 1) / params.batchSize) + 1, ceil(numel(subset) / params.batchSize));
...
if isempty(whos('stats'))
  error('Epoch ''%s'' was only partially saved. Delete this file and try again.', fileName);
end
